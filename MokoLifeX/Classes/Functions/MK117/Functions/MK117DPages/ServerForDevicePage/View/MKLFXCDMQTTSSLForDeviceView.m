...
@implementation MKLFXCDMQTTSSLForDeviceViewModel
@end

@interface MKLFXCDMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKLFXCDMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(lfxc_mqtt_sslParams_device117D_caFilePressed)]) {
            [self.delegate lfxc_mqtt_sslParams_device117D_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(lfxc_mqtt_sslParams_device117D_clientKeyPressed)]) {
            [self.delegate lfxc_mqtt_sslParams_device117D_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(lfxc_mqtt_sslParams_device117D_clientCertPressed)]) {
        [self.delegate lfxc_mqtt_sslParams_device117D_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(lfxc_mqtt_sslParams_device117D_sslStatusChanged:)]) {
        [self.delegate lfxc_mqtt_sslParams_device117D_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(lfxc_mqtt_sslParams_device117D_certificateChanged:)]) {
            [self.delegate lfxc_mqtt_sslParams_device117D_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKLFXCDMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKLFXCDMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MokoLifeX", @"MKLFXCDMQTTSSLForDeviceView", @"lfx_switchSelectedIcon.png") : LOADICON(@"MokoLifeX", @"MKLFXCDMQTTSSLForDeviceView", @"lfx_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MokoLifeX", @"MKLFXCDMQTTSSLForDeviceView", @"lfx_switchUnselectedIcon.png") forState:UIControlStateNormal];
