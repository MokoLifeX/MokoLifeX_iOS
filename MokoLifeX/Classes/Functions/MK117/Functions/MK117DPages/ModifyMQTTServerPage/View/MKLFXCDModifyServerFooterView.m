//
//  MKLFXCDModifyServerFooterView.m
//  MokoLifeX_Example
//
...
@implementation MKLFXCDModifyServerFooterViewModel
@end
...
@interface MKLFXCDModifyServerFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKLFXCDModifyServerSSLViewDelegate,
MKLFXCDModifyServerSettingViewDelegate>
...
@property (nonatomic, strong)MKLFXCDModifyServerSSLView *sslView;

@property (nonatomic, strong)MKLFXCDModifyServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKLFXCDModifyServerFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_qosChanged:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKLFXCDModifyServerSSLViewDelegate
- (void)lfxc_117d_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)lfxc_117d_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_certificateChanged:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_certificateChanged:certificate];
    }
}
...
- (void)lfxc_117d_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:value textID:(index + 5)];
    }
}

#pragma mark - MKLFXCDModifyServerSettingViewDelegate
- (void)lfxc_117d_mqtt_modifyDevice_wifiSSIDChanged:(NSString *)ssid {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:ssid textID:3];
    }
}

- (void)lfxc_117d_mqtt_modifyDevice_wifiPasswordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate lfxc_117d_mqtt_modifyMQTT_textFieldValueChanged:password textID:4];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKLFXCDModifyServerFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKLFXCDModifyServerFooterViewModel.class]) {
        return;
    }
    ...
    MKLFXCDModifyServerSSLViewModel *sslModel = [[MKLFXCDModifyServerSSLViewModel alloc] init];
    ...
    MKLFXCDModifyServerSettingViewModel *settingModel = [[MKLFXCDModifyServerSettingViewModel alloc] init];
...
- (MKLFXCDModifyServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKLFXCDModifyServerSSLView alloc] init];
...
- (MKLFXCDModifyServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKLFXCDModifyServerSettingView alloc] init];
