...
@implementation MKLFXMQTTSSLForAppViewModel
@end

@interface MKLFXMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKLFXMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate lfx_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate lfx_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate lfx_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate lfx_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKLFXMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKLFXMQTTSSLForAppViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MokoLifeX", @"MKLFXMQTTSSLForAppView", @"lfx_switchSelectedIcon.png") : LOADICON(@"MokoLifeX", @"MKLFXMQTTSSLForAppView", @"lfx_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MokoLifeX", @"MKLFXMQTTSSLForAppView", @"lfx_switchUnselectedIcon.png") forState:UIControlStateNormal];
