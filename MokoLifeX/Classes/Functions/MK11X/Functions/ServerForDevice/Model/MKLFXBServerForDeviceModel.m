...
@interface MKLFXBServerForDeviceModel ()
...
@implementation MKLFXBServerForDeviceModel
...
- (void)configParamsWithWifiSSID:(NSString *)ssid
    ...
    dispatch_async(self.configQueue, ^{
        if (![MKLFXBSocketInterface shared].isConnected) {
...
- (BOOL)connectDevice {
    ...
    [[MKLFXBSocketInterface shared] connectWithSucBlock:^{
        success = YES;
...
- (BOOL)configMQTTServer {
    __block BOOL success = NO;
    lfxb_mqttServerConnectMode connectMode = lfxb_mqttServerConnectTCPMode;
    if (self.sslIsOn) {
        if (self.certificate == 0 || self.certificate == 1) {
            connectMode = lfxb_mqttServerConnectOneWaySSLMode;
        }else if (self.certificate == 2) {
            connectMode = lfxb_mqttServerConnectTwoWaySSLMode;
        }
    }
    [[MKLFXBSocketInterface shared] lfxb_configMQTTServerHost:self.host
...
- (BOOL)configTopics {
    ...
    [[MKLFXBSocketInterface shared] lfxb_configSubscibeTopic:subscribeTopic
...
- (BOOL)configDeviceID {
    __block BOOL success = NO;
    [[MKLFXBSocketInterface shared] lfxb_configDeviceID:self.deviceID
                                               sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
