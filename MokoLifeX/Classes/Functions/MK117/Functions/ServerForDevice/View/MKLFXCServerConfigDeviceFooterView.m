...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_qosChanged:)]) {
        [self.delegate lfx_mqtt_serverForDevice_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}
...
- (void)lfx_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)lfx_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate lfx_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)lfx_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate lfx_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)lfx_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate lfx_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)lfx_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate lfx_mqtt_serverForDevice_fileButtonPressed:2];
    }
}
...
- (void)lfx_mqtt_deviecSetting_deviceIDChanged:(NSString *)deviceID {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_textFieldValueChanged:deviceID textID:3];
    }
}

- (void)lfx_mqtt_deviecSetting_ntpURLChanged:(NSString *)url {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate lfx_mqtt_serverForDevice_textFieldValueChanged:url textID:4];
    }
}

- (void)lfx_mqtt_deviecSetting_timeZoneChanged:(NSInteger)timeZone {
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_serverForDevice_timeZoneChanged:)]) {
        [self.delegate lfx_mqtt_serverForDevice_timeZoneChanged:timeZone];
    }
}
