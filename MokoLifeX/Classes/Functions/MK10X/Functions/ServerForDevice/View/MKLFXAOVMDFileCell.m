...
@implementation MKLFXAOVMDFileCellModel
@end

@interface MKLFXAOVMDFileCell ()
...
@implementation MKLFXAOVMDFileCell

+ (MKLFXAOVMDFileCell *)initCellWithTableView:(UITableView *)tableView {
    MKLFXAOVMDFileCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKLFXAOVMDFileCellIdenty"];
    if (!cell) {
        cell = [[MKLFXAOVMDFileCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKLFXAOVMDFileCellIdenty"];
    }
    return cell;
}
...
- (void)fileButtonPressed {
    if ([self.delegate respondsToSelector:@selector(lfxa_certFileButtonPressed:)]) {
        [self.delegate lfxa_certFileButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKLFXAOVMDFileCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKLFXAOVMDFileCellModel.class]) {
        return;
    }
...
- (UIButton *)fileButton {
    if (!_fileButton) {
        ...
        [_fileButton setImage:LOADICON(@"MokoLifeX", @"MKLFXAOVMDFileCell", @"lfx_config_certAddIcon.png") forState:UIControlStateNormal];
