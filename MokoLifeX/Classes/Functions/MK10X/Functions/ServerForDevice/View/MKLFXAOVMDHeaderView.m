...
@interface MKLFXAOVMDHeaderConnectModeButton : UIControl
...
@implementation MKLFXAOVMDHeaderConnectModeButton
...
@interface MKLFXAOVMDHeaderQosButton : UIControl
...
@implementation MKLFXAOVMDHeaderQosButton
...
- (UIImageView *)icon {
    if (!_icon) {
        _icon = [[UIImageView alloc] init];
        _icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderQosButton", @"lfx_triangleIcon.png");
    }
    return _icon;
}
...
@interface MKLFXAOVMDHeaderView ()
...
@property (nonatomic, strong)MKLFXAOVMDHeaderQosButton *qosButton;
...
@property (nonatomic, strong)MKLFXAOVMDHeaderConnectModeButton *tcpButton;

@property (nonatomic, strong)MKLFXAOVMDHeaderConnectModeButton *oneWayButton;

@property (nonatomic, strong)MKLFXAOVMDHeaderConnectModeButton *twoWayButton;

@property (nonatomic, assign)NSInteger connectMode;

@end

@implementation MKLFXAOVMDHeaderView
...
- (void)cleanSessionButtonPressed {
    ...
    UIImage *cleanIcon = (self.cleanSessionButton.selected ? LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_switchSelectedIcon.png") : LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_switchUnselectedIcon.png"));
    [self.cleanSessionButton setImage:cleanIcon forState:UIControlStateNormal];
    [self resignAllTextField];
    if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewCleanSessionChanged:)]) {
        [self.delegate lfxa_ovmdHeaderViewCleanSessionChanged:self.cleanSessionButton.selected];
    }
}
...
- (void)qosButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewQosChanged:)]) {
            [self.delegate lfxa_ovmdHeaderViewQosChanged:currentRow];
        }
    }];
}
...
- (void)tcpButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewConnectModeChanged:)]) {
        [self.delegate lfxa_ovmdHeaderViewConnectModeChanged:0];
    }
}
...
- (void)oneWayButtonPressed {
    ...
    [self updateConnectMode];
    if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewConnectModeChanged:)]) {
        [self.delegate lfxa_ovmdHeaderViewConnectModeChanged:1];
    }
}
...
- (void)twoWayButtonPressed {
    ...
    [self updateConnectMode];
    if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewConnectModeChanged:)]) {
        [self.delegate lfxa_ovmdHeaderViewConnectModeChanged:2];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKLFXAOVMDHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKLFXAOVMDHeaderViewModel.class]) {
        return;
    }
    ...
    UIImage *cleanIcon = (_dataModel.cleanSession ? LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_switchSelectedIcon.png") : LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_switchUnselectedIcon.png"));
    [self.cleanSessionButton setImage:cleanIcon forState:UIControlStateNormal];
...
- (void)updateConnectMode {
    if (self.connectMode == 0) {
        //tcp
        self.tcpButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeSelectedIcon.png");
        self.oneWayButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeUnselectedIcon.png");
        self.twoWayButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeUnselectedIcon.png");
        return;
    }
    if (self.connectMode == 1) {
        //one-way SSL
        self.tcpButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeUnselectedIcon.png");
        self.oneWayButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeSelectedIcon.png");
        self.twoWayButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeUnselectedIcon.png");
        return;
    }
    if (self.connectMode == 2) {
        ...
        self.tcpButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeUnselectedIcon.png");
        self.oneWayButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeUnselectedIcon.png");
        self.twoWayButton.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_connectModeSelectedIcon.png");
        return;
    }
}
...
- (MKTextField *)hostTextField {
    if (!_hostTextField) {
        ...
        _hostTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:0];
            }
        };
    }
...
- (MKTextField *)portTextField {
    if (!_portTextField) {
        ...
        _portTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:1];
            }
        };
    }
...
- (UIButton *)cleanSessionButton {
    if (!_cleanSessionButton) {
        ...
        [_cleanSessionButton setImage:LOADICON(@"MokoLifeX", @"MKLFXAOVMDHeaderView", @"lfx_switchSelectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:2];
            }
        };
    }
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:3];
            }
        };
    }
...
- (MKLFXAOVMDHeaderQosButton *)qosButton {
    if (!_qosButton) {
        _qosButton = [[MKLFXAOVMDHeaderQosButton alloc] init];
...
- (MKTextField *)keepAliveTextField {
    if (!_keepAliveTextField) {
        ...
        _keepAliveTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:4];
            }
        };
    }
...
- (MKTextField *)clientIDTextField {
    if (!_clientIDTextField) {
        ...
        _clientIDTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:5];
            }
        };
    }
...
- (MKTextField *)deviceIDTextField {
    if (!_deviceIDTextField) {
        ...
        _deviceIDTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(lfxa_ovmdHeaderViewTextFieldValueChanged:index:)]) {
                [self.delegate lfxa_ovmdHeaderViewTextFieldValueChanged:text index:6];
            }
        };
    }
...
- (MKLFXAOVMDHeaderConnectModeButton *)tcpButton {
    if (!_tcpButton) {
        _tcpButton = [[MKLFXAOVMDHeaderConnectModeButton alloc] init];
...
- (MKLFXAOVMDHeaderConnectModeButton *)oneWayButton {
    if (!_oneWayButton) {
        _oneWayButton = [[MKLFXAOVMDHeaderConnectModeButton alloc] init];
...
- (MKLFXAOVMDHeaderConnectModeButton *)twoWayButton {
    if (!_twoWayButton) {
        _twoWayButton = [[MKLFXAOVMDHeaderConnectModeButton alloc] init];
