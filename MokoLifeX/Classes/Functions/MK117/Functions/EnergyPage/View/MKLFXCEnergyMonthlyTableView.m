//
//  MKLFXCEnergyMonthlyTableView.m
//  MokoLifeX_Example
//
...
@interface MKLFXCEnergyMonthlyTableView ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKLFXCEnergyMonthlyTableView

- (void)dealloc {
    NSLog(@"MKLFXCEnergyMonthlyTableView销毁");
...
- (void)receiveCurrentEnergy:(NSNotification *)note {
    ...
    NSArray *startList = [self.startTime componentsSeparatedByString:@"-"];
    self.headerModel.dateMsg = [NSString stringWithFormat:@"%@-%@ to %@-%@",startList[1],startList[2],energyDic[@"date"][@"month"],energyDic[@"date"][@"day"]];
...
- (void)updateEnergyDatas:(NSArray *)energyList
    ...
    NSArray *startList = [startTime componentsSeparatedByString:@"-"];
    NSArray *endList = [endTime componentsSeparatedByString:@"-"];
    self.headerModel.dateMsg = [NSString stringWithFormat:@"%@-%@ to %@-%@",startList[1],startList[2],endList[1],endList[2]];
    [self.tableHeaderView setDataModel:self.headerModel];
}
