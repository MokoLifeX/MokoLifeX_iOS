//
//  MKLFXCDServerForDeviceController.m
//  MokoLifeX_Example
//
...
@interface MKLFXCDServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKLFXCDServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MKLFXCMQTTManagerDeviceOnlineDelegate>
...
@property (nonatomic, strong)MKLFXCDServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKLFXCDServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKLFXCDServerConfigDeviceFooterViewModel *sslParamsModel;
...
@implementation MKLFXCDServerForDeviceController

- (void)dealloc {
    NSLog(@"MKLFXCDServerForDeviceController销毁");
...
#pragma mark - MKLFXCDServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)lfxc_mqtt_serverForDevice_117D_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)lfxc_mqtt_serverForDevice_117D_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)lfxc_mqtt_serverForDevice_117D_qosChanged:(NSInteger)qos {
...
- (void)lfxc_mqtt_serverForDevice_117D_certificateChanged:(NSInteger)certificate {
...
- (void)lfxc_mqtt_serverForDevice_117D_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)lfxc_mqtt_serverForDevice_117D_timeZoneChanged:(NSInteger)timeZone {
...
/// 频段改变
/// @param domain 频段
- (void)lfxc_mqtt_serverForDevice_117D_domainChanged:(NSInteger)domain {
...
#pragma mark - MKLFXCMQTTManagerDeviceOnlineDelegate
- (void)lfxc_deviceOnline:(NSString *)deviceID {
    ...
    [MKLFXCMQTTManager shared].delegate = nil;
    if (self.connectTimer) {
        dispatch_cancel(self.connectTimer);
    }
    self.timeCount = 0;
...
- (void)configServerParamsComplete {
    NSString *topic = @"";
    if (ValidStr([MKLFXCMQTTManager shared].subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKLFXCMQTTManager shared].subscribeTopic;
    }else {
        topic = [self.dataModel currentPublishTopic];
    }
    [self.progressView show];
    [[MKLFXCMQTTManager shared] subscriptions:@[topic]];
    [MKLFXCMQTTManager shared].delegate = self;
...
- (void)connectSuccess {
    ...
    [MKLFXCSocketInterface sharedDealloc];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MokoLifeX", @"MKLFXCDServerForDeviceController", @"lfx_saveIcon.png") forState:UIControlStateNormal];
...
- (MKLFXCDServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKLFXCDServerForDeviceModel alloc] init];
...
- (MKLFXCDServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKLFXCDServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKLFXCDServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKLFXCDServerConfigDeviceFooterViewModel alloc] init];
    }
