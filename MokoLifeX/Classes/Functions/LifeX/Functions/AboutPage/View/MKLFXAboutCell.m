...
@implementation MKLFXAboutCellModel
@end

@interface MKLFXAboutCell ()
...
@implementation MKLFXAboutCell

+ (MKLFXAboutCell *)initCellWithTableView:(UITableView *)table {
    MKLFXAboutCell *cell = [table dequeueReusableCellWithIdentifier:@"MKLFXAboutCellIdenty"];
    if (!cell) {
        cell = [[MKLFXAboutCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKLFXAboutCellIdenty"];
    }
    return cell;
}
...
#pragma mark -
- (void)setDataModel:(MKLFXAboutCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKLFXAboutCellModel.class]) {
        return;
    }
    self.typeLabel.text = _dataModel.typeMessage;
    self.valueLabel.text = _dataModel.value;
    self.icon.image = LOADICON(@"MokoLifeX", @"MKLFXAboutCell", _dataModel.iconName);
