...
@interface MKLFXAMoreController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKLFXAMoreController

- (void)dealloc {
    NSLog(@"MKLFXAMoreController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 1) {
        ...
        vc.protocol = [[MKLFXADeviceInfoDataModel alloc] init];
    ...
    if (indexPath.row == 2) {
        ...
        vc.deviceModel = self.deviceModel;
        vc.protocol = [[MKLFXAUpdateDataModel alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.row == 3) {
        ...
        vc.deviceModel = self.deviceModel;
        vc.protocol = [[MKLFXAPowerOnStatusModel alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)removeButtonPressed {
    ...
    alertView.notificationName = @"mk_lfxa_dismissAlert";
    @weakify(self);
...
- (void)resetDevice {
    ...
    [MKLFXAMQTTInterface lfxa_resetDeviceWithDeviceID:self.deviceModel.deviceID topic:[self.deviceModel currentSubscribedTopic] sucBlock:^{
        [[MKHudManager share] hide];
        [self deleteDeviceFormLocal];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
