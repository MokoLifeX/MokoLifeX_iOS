...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate lfx_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate lfx_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate lfx_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate lfx_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(lfx_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate lfx_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}
