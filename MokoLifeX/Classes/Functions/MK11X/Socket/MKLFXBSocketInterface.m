...
static MKLFXBSocketInterface *manager = nil;
static dispatch_once_t onceToken;


static NSInteger const certPackageDataLength = 200;

@interface MKLFXBSocketInterface ()
...
@implementation MKLFXBSocketInterface

+ (MKLFXBSocketInterface *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKLFXBSocketInterface new];
        }
    });
    return manager;
}
...
- (void)lfxb_configMQTTServerHost:(NSString *)host
    ...
    if (qos == lfxb_mqttQosLevelAtMostOnce) {
        qosNumber = 0;
    }else if (qos == lfxb_mqttQosLevelAtLeastOnce){
        qosNumber = 1;
    }
    NSInteger connectModel = 0;
    if (mode == lfxb_mqttServerConnectOneWaySSLMode) {
        connectModel = 1;
    }else if (mode == lfxb_mqttServerConnectTwoWaySSLMode) {
        connectModel = 3;
    }
    NSDictionary *json = @{
        @"header":@(4002),
    ...
    [[MKLFXSocketManager shared] addTaskWithTag:lfxb_socket_configMQTTServerTag
                                           data:jsonString
                                       sucBlock:sucBlock
                                    failedBlock:failedBlock];
}
...
- (void)lfxb_configSubscibeTopic:(NSString *)subscibeTopic
    ...
    NSString *jsonString = [MKLFXSocketAdopter convertToJsonData:json];
    [[MKLFXSocketManager shared] addTaskWithTag:lfxb_socket_configTopicTag
                                           data:jsonString
                                       sucBlock:sucBlock
                                    failedBlock:failedBlock];
}
...
- (void)lfxb_configElectricalDefaultState:(lfxb_electricalDefaultState)state
    ...
    NSString *jsonString = [MKLFXSocketAdopter convertToJsonData:json];
    [[MKLFXSocketManager shared] addTaskWithTag:lfxb_socket_configElectricalDefaultStateTag
                                           data:jsonString
                                       sucBlock:sucBlock
                                    failedBlock:failedBlock];
}
...
- (void)lfxb_configWifiSSID:(NSString *)ssid
    ...
    if (security == lfxb_wifiSecurity_WEP) {
        wifi_security = 1;
    }else if (security == lfxb_wifiSecurity_WPA_PSK){
        wifi_security = 2;
    }else if (security == lfxb_wifiSecurity_WPA2_PSK){
        wifi_security = 3;
    }else if (security == lfxb_wifiSecurity_WPA_WPA2_PSK){
        wifi_security = 4;
    }
    NSDictionary *json = @{
        @"header":@(4006),
    ...
    NSString *jsonString = [MKLFXSocketAdopter convertToJsonData:json];
    [[MKLFXSocketManager shared] addTaskWithTag:lfxb_socket_configWifiParamsTag
                                           data:jsonString
                                       sucBlock:sucBlock
                                    failedBlock:failedBlock];
}
...
- (void)lfxb_configDeviceID:(NSString *)deviceID
    ...
    NSString *jsonString = [MKLFXSocketAdopter convertToJsonData:json];
    [[MKLFXSocketManager shared] addTaskWithTag:lfxb_socket_configDeviceIDTag
                                           data:jsonString
                                       sucBlock:sucBlock
                                    failedBlock:failedBlock];
}
...
- (BOOL)sendCertData:(NSString *)data {
    ...
    [[MKLFXSocketManager shared] addTaskWithTag:lfxb_socket_configCertDataTag data:data sucBlock:^(id  _Nonnull returnData) {
        success = YES;
